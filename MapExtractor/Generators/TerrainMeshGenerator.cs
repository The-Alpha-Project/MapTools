        ...
        public static void BuildTerrainMesh(CMapObj map)
        {
            for (int tileBlockX = 0; tileBlockX < Constants.TileBlockSize; tileBlockX++)
            {
                for (int tileBlockY = 0; tileBlockY < Constants.TileBlockSize; tileBlockY++)
                {
                    ...
                    // NAbey
                    //if ((tileBlockX != 47 && tileBlockX != 48 && tileBlockX != 49) || (tileBlockY != 30 && tileBlockY != 31 && tileBlockY != 32))
                    //    continue;
                    ...
                    AppendWMOandMDXInfo(tileArea, ref tileVerts, ref tileIndices);
                    ExportMesh(outputFileName, tileVerts.ToRecastVerts(), tileIndices.ToArray());
                }
            }
        }

        private static void AppendWMOandMDXInfo(CMapArea area, ref List<Vector3> verts, ref List<int> indices)
        {
            // TODO
        }

        private static void ExportMesh(string fileName, IEnumerable<Vector3> vertices, int[] indices)
        {
