...
namespace MapTools.Helpers
{
    public static class Paths
    {
        ...
        private static string _cacheOutputMeshPath = string.Empty;
        public static string OutputMeshPath
        {
            get
            {
                if (!string.IsNullOrEmpty(_cacheOutputMeshPath))
                {
                    if (!Directory.Exists(_cacheOutputMeshPath))
                        Directory.CreateDirectory(_cacheOutputMeshPath);

                    return _cacheOutputMeshPath;
                }

                _cacheOutputMeshPath = Paths.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), @"meshes\");
